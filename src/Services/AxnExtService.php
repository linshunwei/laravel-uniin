 ...
 * AXN+分机号 模式
 */
class AxnExtService extends BaseService
{
	/**
	 * @param array $data
	 ...
	 * extension String 可选 分机号，如果此字段未提供，则平台会自定分配一个分机号返回，1~4 位；注意：对于同一个 X 号码，要么都指定分机号，要么都不指定统一让平台生成。
	...
	public function stdRelease(string $mappingId = ''): mixed
	{
		...
		return $this->post('/Accounts/' . $this->config['account_sid'] . '/axnext/std/release', $data);
	}
	...
	public function query(string $mappingId = ''): mixed
	{
		...
		return $this->post('/Accounts/' . $this->config['account_sid'] . '/axnext/std/query', $data);
	}
