...
class SM4
{
	...
	public function encrypt($key, $data, $replace = 0)
	{
		...
		$bytes = $this->pad($data, $this->_blockSize);
		$chunks = array_chunk($bytes, $this->_blockSize);
	...
	public function decrypt($key, $data, $replace = 1)
	{
		...
		if (strlen($data) < 0 || strlen($data) % $this->_blockSize != 0) {
			return false;
		}
		...
		if($replace == 1) {
			...
			$plaintext = str_replace("%2F", "/", $plaintext);
		}
		return $plaintext;
	}
	...
	private function pad($data)
	{
		...
		$rem = $this->_blockSize - count($bytes) % $this->_blockSize;
		for ($i = 0; $i < $rem; $i++) {
