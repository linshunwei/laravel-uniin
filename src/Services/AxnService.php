 ...
 * axn 模式
 */
class AxnService extends BaseService
{
	/**
	 * @param array $data
	 ...
	 * autoaxbBind String 可选当N呼叫X转接到A时，是否自动建立axb的绑定关系，便于A可以回呼到N。0：否，1：是，默认值为0
	...
	public function stdRelease(string $mappingId = ''): mixed
	{
		...
		return $this->post('/Accounts/' . $this->config['account_sid'] . '/axn/std/release', $data);
	}

	/**
	 * 绑定更新
	 * @param string $mappingId
	 * @param string $bNumber 用户A固定回呼的电话号码
	...
	public function stdUpdate(string $mappingId, string $bNumber = '', int $mappingDuration = null, int $autoBandingDuration = null)
	{
		...
		if (!is_null($autoBandingDuration)) $data['autoBandingDuration'] = $autoBandingDuration;
		return $this->post('/Accounts/' . $this->config['account_sid'] . '/axn/std/set', $data);
	}
	...
	public function query(string $mappingId = ''): mixed
	{
		...
		return $this->post('/Accounts/' . $this->config['account_sid'] . '/axn/std/query', $data);
	}
